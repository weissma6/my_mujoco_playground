...
preFrmNo = 0
curFrmNo = 0
iFrameRate = 60  # 默认帧率，将在 py_desc_func 中更新
global client
...
def py_data_func(pFrameOfMocapData, pUserData):
    if pFrameOfMocapData == None:  
        ...
        global preFrmNo, curFrmNo, iFrameRate
        ...
        for iMarkerSet in range(frameData.nMarkerSets):
            ...
            for iMarker in range(markerset.nMarkers):
                ...
                # 计算速度
                vel_method = CalculateVelocity(iFrameRate, 3)  # FPS:60 FrameFactor:3
                velocity = velocity_arrays[marker_key].try_to_calculate(vel_method)
                
                # 计算加速度
                acc_method = CalculateAcceleration(iFrameRate, 3)
...
def py_desc_func(pdds):
    ...
    for iDef in range(dataDefs.nDataDescriptions):
        ...
        if dataDef.type == DataDescriptors.Descriptor_Skeleton.value:
            ...
            iFrameRate = dataParam.nFrameRate
            print(f'FrameRate:{iFrameRate}Hz')
        elif dataDef.type == DataDescriptors.Descriptor_CameraParam.value:
            cameraDescDef = dataDef.Data.cameraDescriptions.contents
            nCameras = cameraDescDef.nCameras
            print(f'Camera Count: {nCameras}')
            for index in range(nCameras):
                cameraData = cameraDescDef.Cameras[index]
                print(f"No:{cameraData.cameraId} x:{cameraData.tx} y:{cameraData.ty} z:{cameraData.tz} "\
                      f"rx:{cameraData.rx}  ry:{cameraData.ry}  rz:{cameraData.rz}")
...
def main(argv):
    ...
    print('NokovSDK Sample Client 4.1.0.5873(NokovSDK ver. %d.%d.%d.%d)' % (ver[0], ver[1], ver[2], ver[3]))
