...
sbatch --array=1 batch_runs/slurm/run_array_ur3.sbatch   # pickOrient4cm_smoke (line 1 of sweep)
sbatch --array=2 batch_runs/slurm/run_array_ur3.sbatch   # pickOrient4cm_base baseline (line 2)
sbatch --array=1-3 batch_runs/slurm/run_array_ur3.sbatch  # full sweep (3 configs)
# Sweep config: batch_runs/sweeps/UR3Pick_sweep.jsonl (1-indexed by SLURM array task).
# Lines: 1 smoke, 2 base, 3 lr_low_longunroll (lr=5e-4, unroll=50, seed 1).
...
- `manipulation/my_ur3/` — Custom UR3e + Robotiq Hand-E pick environment (`ur3_pick` branch), mirroring `my_ur10/`:
  ...
  - `ur3_pick.py` — `UR3Pick` env. **Pick + lift task** (drop/place stage deferred). **26D obs** `[q(8 arm+finger), qd(6 arm), (box−tcp)(3), (target_pos−box)(3), box_xmat.ravel()[:6](6)]` (the last 6 are the first two rows of the box rotation matrix), **7D action** (6 arm delta + 1 Hand-E tendon). `ctrl_dt=0.02`, `sim_dt=0.005`, `action_scale=0.04`, `episode_length=150`, `init_keyframe="low_home"`. Reward scales `{box_target:8.0, gripper_box:4.0, box_orient:2.0, finger_touch:1.0, no_floor_collision:0.25, robot_target_qpos:0.3}`. `box_orient` = `(1 − tanh(2·rot_err)) · reached_box` with `rot_err = ||eye(3).ravel()[:6] − box_xmat.ravel()[:6]||` (box vs **canonical/identity** orientation), gated by the sticky `reached_box` flag. `target_pos` is sampled at reset as a **lift point in the air above the box** (`init_obj_pos + uniform([-0.1,-0.1,0.10], [0.1,0.1,0.25])`), written to `mocap_pos` for viz, and stored in `info["target_pos"]`. Reset also bakes a **random Y-axis box tilt** `θ ∈ (−π/4, +π/4)` into `qpos[obj_qposadr+3:+7]` as `(cos(θ/2), 0, sin(θ/2), 0)`. `info["reached_box"]` is a sticky gate that latches at `gripper_box_dist < 0.02`. Success: `||box − target_pos|| < 0.03` for 3 consecutive steps. `out_of_bounds`: `|tcp.xy| > 0.6` (UR3 reach ≈ 0.5 m).
  ...
  - `xmls/mjx_single_cube_position_ur3.xml` — pick scene: includes the robot + scene, **4×4×4 cm box** freejoint (geom half-extents `0.02`, resting center z=0.02), `mocap_target` body (matching 0.02 half-extent for viz), `ur3_pick_sensor.xml`. Loads to `nu=7, nq=15, nkey=5, nsensor=8`. Keyframes `task_home`/`low_home`/`tucked` (15 qpos = 6 arm + 2 finger + 7 box; 7 ctrl); box pose `(0.3, 0, 0.02)` identity orientation.
...
- `sweeps/UR3Pick_sweep.jsonl` — 3 lines: `pickOrient4cm_smoke` (300k/512 envs, seed 0), `pickOrient4cm_base` (20M baseline, lr=1e-3, unroll=10, seed 0), `pickOrient4cm_lr_low_longunroll` (20M, lr=5e-4, unroll=50, seed 1). All `env_name="UR3Pick"`, `wandb_project="UR3_pick_ppo"`, `camera_kwargs={"camera":"box_detail"}`. The two real runs tag `cube4cm`/`rot_reward`/`rand_orient`. (Note the W&B project name is intentionally `UR3_pick_ppo` — case-sensitive — not the lowercase `ur3_pick_ppo`.)
...
- `ur3_realrobot_dependencies.py` — `UR3RealRobotPick`, the UR3 pick analog of `URSim_RTDE_dependencies.py`. Key differences:
  ...
  - `run_policy_loop(drop_target, mocap_reader, …, gripper_fn, use_fk_tcp)` reads `mocap_reader.get_rigid_body_xyz()` **and** `get_rigid_body_quat()` every tick (orientation feeds the 26D obs internally; the user only places the box), each falling back to the last good value when a frame is missing.
- `ur3_realrobot_pickloop.py` — single-run UR3 pick entry (mirror of `UR10_RealRobot_Reach_ONE.py`). Config at top: `ROBOT_IP`, `DROP_TARGET` (lift target), `Q_START` (UR3 `low_home`), `MOCAP_SERVER_IP`/`MOCAP_RIGID_BODY_ID`, `WANDB_RUN_ID`/`WANDB_ENTITY`/`WANDB_PROJECT` (auto-downloads the policy into `POLICY_PATH` if absent), `ENABLE_GRIPPER` (False = no-op gripper for URSim dry-runs). For a dry-run set `ROBOT_IP=127.0.0.1` and leave `ENABLE_GRIPPER=False`.
