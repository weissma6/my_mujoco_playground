...
sbatch --array=1 batch_runs/slurm/run_array_ur3.sbatch    # config 1 (env2048, 20M)
...
# 5 lines (pickPlace_*): fixed lr=6.25e-4, unroll=20, seed 0, wandb_project=UR3_PicknDrop;
# scales num_envs (2048/4096/8192) and num_timesteps (20M/20M/20M/30M/40M).
...
- `sweeps/UR3Pick_sweep.jsonl` — **5 lines** (`pickPlace_un20_env{N}_{M}M`): a **scaling** sweep, not a hyperparameter sweep — fixed `learning_rate=6.25e-4`, `unroll_length=20`, seed 0, `num_evals=15`; varies `num_envs` **2048/4096/8192** and `num_timesteps` **20M/20M/20M/30M/40M**. All `env_name="UR3Pick"`, `wandb_project="UR3_PicknDrop"` (case-sensitive), `camera_kwargs={"camera":"box_detail"}`, tags `cube4cm`/`place`/`rand_orient`/`un20`/`env<N>`/`<M>M`. (Older pick policies still live in the separate `UR3_pick_ppo` project; the deployment `download_policy_from_wandb` default still points there.)
