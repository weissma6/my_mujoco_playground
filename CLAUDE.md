...
# .submit/.sbatch files HARD-CODE --array (e.g. ur3_jsonl_array.submit=1-4%2, <=2 concurrent)
...
- `manipulation/my_ur3/` — Custom UR3e + Robotiq Hand-E pick environment, mirroring `my_ur10/`:
  ...
  - `ur3_pick.py` — `UR3Pick` env. **Lift-to-an-air-point task** — NOT pick-and-place (that is the sibling `ur3_picknplace.py`; do not conflate them). The box spawns near the keyframe pose with **wide XY jitter** (`±[0.15, 0.10]`, so X∈[0.30,0.60] at `task_home`'s 0.45) and a random **±45° Y-tilt**; the policy grasps it and lifts it to a **target point floating ~8–15 cm above** (rendered as a red **sphere** marker), holding within **5 mm**. **26D obs** `[q(8 arm+finger), qd(6 arm), (box−tcp)(3), (target_pos−box)(3), box_xmat.ravel()[:6](6)]`, **7D action** (6 arm delta + 1 Hand-E tendon). `ctrl_dt=0.02`, `sim_dt=0.005`, `action_scale=0.04`, `episode_length=150`, `init_keyframe="low_home"` (sweeps override to `task_home`). **Reward = 6 ungated terms** (no staged latches, no containment), scales `{box_target:8.0, gripper_box:4.0, box_orient:2.0, finger_touch:1.0, no_floor_collision:0.25, robot_target_qpos:0.3}`. `box_target` is **two-scale** `0.5·(1−tanh 5d) + 0.5·(1−tanh 30d)` — the fine term supplies a steep near-goal gradient so the box is pulled the last ~cm (plain `tanh 5d` is flat near zero, which had the box plateauing ~13 mm short).
    - **Success** = box-center within `success_tol` (config, default **0.005 m**) of the target for **3 consecutive steps**; episode also ends on `out_of_bounds` (`|tcp.xy| > 0.6` or `tcp.z < 0`) or NaN. There is **no** corner/containment math, `box_inside`, `grasp`, `lift`, `release`, `retract`, or sticky latch here — all of that lives **only** in `ur3_picknplace.py`. `rot_err` is computed but diagnostic-only (not in success, not in metrics).
    - **Reset** (`reset()`): box `init_obj_pos + uniform(±[0.15, 0.10, 0])` with the ±45° Y-tilt baked into the quat; target `init_obj_pos + uniform([+0.01,−0.03,+0.08]‥[0.06,0.03,0.15])` (X biased forward; envelope kept **reachable** — required for the 5 mm goal; Y pulled in), written to `mocap_pos` for viz and to `info["target_pos"]`. Per-joint reset noise from `init_qpos_noise` — a **7-vector** of per-direction amplitudes (rad), 6 arm + 1 finger, applied `uniform(−v,+v)` on the keyframe; **index 0 = base joint, index 6 = finger** (the two are the usual sweep knobs).
  - `ur3_picknplace.py` — `UR3PicknPlace` env (registered alongside `UR3Pick` in `manipulation/__init__.py`). The **dedicated pick-and-place sibling** of `ur3_pick.py`, larger/more developed (~560 vs ~389 lines), and the env the `UR3PicknPlace_sweep.jsonl` → `UR3_PicknDrop` project actually trains. Same `UR3Base`, same staged sticky-latch reward (approach→grasp→lift→deliver) and 26D obs / 7D action layout as `UR3Pick`; loads its **own** scene `xmls/mjx_single_cube_position_ur3_picknplace.xml` and defaults `init_keyframe="low_home"` (the sweep overrides to `task_home`).
  ...
  - `xmls/mjx_single_cube_position_ur3.xml` — pick scene: includes the robot + scene, **4×4×4 cm box** freejoint (geom half-extents `0.02`, resting center z=0.02), `mocap_target` body (a **visual sphere** geom, **radius `0.04`** — big enough to contain the 4 cm cube so a lifted box visibly sits inside it; `contype/conaffinity=0`), `ur3_pick_sensor.xml`. Loads to `nu=7, nq=15, nkey=5, nsensor=8`. Keyframes `task_home`/`low_home`/`tucked` (15 qpos = 6 arm + 2 finger + 7 box; 7 ctrl), identity box orientation; box pose `(0.45, 0, 0.02)` in `task_home` (the sweep keyframe), `(0.3, 0, 0.02)` in `low_home`/`tucked`. (The 5 cm **box** target with corner-containment is `mjx_single_cube_position_ur3_picknplace.xml`, not this file.)
...
- `batch_runs/sweeps/UR3Pick_sweep.jsonl` — **simple pick** sweep (`env_name="UR3Pick"`, `wandb_project="UR3_pick_ppo"`). Contents are **volatile** (user-edited per experiment) — read the file for the current configs, don't trust a line count here.
