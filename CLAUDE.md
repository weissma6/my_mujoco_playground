...
- `manipulation/my_ur3/` — Custom UR3e + Robotiq Hand-E pick environment (`ur3_pick` branch), mirroring `my_ur10/`:
  ...
  - `ur3_pick.py` — `UR3Pick` env. **Pick + lift task** (drop/place stage deferred). **26D obs** `[q(8 arm+finger), qd(6 arm), (box−tcp)(3), (target_pos−box)(3), box_xmat.ravel()[:6](6)]` (the last 6 are the first two rows of the box rotation matrix), **7D action** (6 arm delta + 1 Hand-E tendon). `ctrl_dt=0.02`, `sim_dt=0.005`, `action_scale=0.04`, `episode_length=150`, `init_keyframe="low_home"`. Reward scales `{box_target:8.0, gripper_box:4.0, box_inside:3.0, finger_touch:1.0, no_floor_collision:0.25, robot_target_qpos:0.3}`. `box_inside` = `(1 − tanh(15·containment_violation)) · reached_box`, where `containment_violation = sum(relu(|box_corners − target_pos| − target_half))` over the **8 world-frame corners** of the 4 cm box (`box_pos + R_box·(±box_half)`) vs the **axis-aligned 5 cm target box** (`target_half = 0.025`); it is zero iff the whole box sits inside the target, so it jointly encodes fine position **and** upright orientation. This **replaces** the old `box_orient` term (`rot_err = ||eye(3).ravel()[:6] − box_xmat.ravel()[:6]||` is kept only as a logged diagnostic). `target_pos` is sampled at reset as a **lift point in the air above the box** (`init_obj_pos + uniform([-0.1,-0.1,0.10], [0.1,0.1,0.25])`), written to `mocap_pos` for viz, and stored in `info["target_pos"]`. Reset also bakes a **random Y-axis box tilt** `θ ∈ (−π/4, +π/4)` into `qpos[obj_qposadr+3:+7]` as `(cos(θ/2), 0, sin(θ/2), 0)`. `info["reached_box"]` is a sticky gate that latches at `gripper_box_dist < 0.02`. Success: every box corner inside the target (`max(corner_overshoot) ≤ 1e-3`) for 3 consecutive steps. `out_of_bounds`: `|tcp.xy| > 0.6` (UR3 reach ≈ 0.5 m).
