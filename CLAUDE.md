...
# Two pipelines, each its own sbatch -> sweep -> W&B project:
sbatch --array=1-2 batch_runs/slurm/run_array_ur3_pick.sbatch        # simple pick:  UR3Pick_sweep.jsonl      -> UR3_pick_ppo
sbatch --array=1-2 batch_runs/slurm/run_array_ur3_picknplace.sbatch  # pick & place: UR3PicknPlace_sweep.jsonl -> UR3_PicknDrop
# Sweeps are 1-indexed by $SLURM_ARRAY_TASK_ID (an index past the line count -> IndexError).
# Both sbatch default to --array=1-6%2 (<=2 concurrent); sweep CONTENTS are volatile
# (edited per experiment) — read the JSONL for the current configs, don't trust counts here.
...
ZHAW SLURM cluster, rootless Podman container (`mujoco_env_EGL.tar`, tag `mujoco:env_EGL`) with `MUJOCO_GL=egl`. A sweep is a JSONL file (one config per line, **1-indexed** by `$SLURM_ARRAY_TASK_ID`); each array task trains one config and uploads a `policy_parameters_*` artifact (type `model`) to W&B. The artifact is **self-describing**: `run_experiment` (`robots/UR10e/UR10_ppo.py`) adds `params.msgpack` + `final_metrics.json` + **`inference_config.json`** (the resolved `network_factory`, obs/action sizes, `env_name`, `episode_length`) — the deploy loader reads that config rather than `run.config` (the sweep JSONL never sets `network_factory`, so it logs as `None` in `run.config`).
- `slurm/run_array_ur10.sbatch` / `run_array_ur3_pick.sbatch` / `run_array_ur3_picknplace.sbatch` — SLURM array jobs. Load the image tar under a `flock`, mount the repo at `/workspace`, run `scripts/run_one_ur{10,3}.py`. `REPO_DIR="$HOME/VT1_MBRL/my_mujoco_playground"`; W&B key read from `$HOME/.secrets/wandb_key`.
...
- `sweeps/UR3Pick_sweep.jsonl` — **simple pick** sweep (`env_name="UR3Pick"`, `wandb_project="UR3_pick_ppo"`). Contents are **volatile** (user-edited per experiment; currently short `Pick_quick_*`/`Pick_tiny_*` diagnostics that vary `num_timesteps`/`unroll_length`/`num_envs`) — read the file for the current configs, don't trust a line count here.
- `sweeps/UR3PicknPlace_sweep.jsonl` — **pick-and-place** sweep (`env_name="UR3PicknPlace"`, `wandb_project="UR3_PicknDrop"`, case-sensitive): **2 lines** (`PicknDrop_un20_env{2048,1024}`), a `num_envs` comparison (2048/1024), not a hyperparameter sweep — fixed `learning_rate=8e-4`, `unroll_length=20`, `num_timesteps=20M`, seed 0, `num_evals=15`, `init_keyframe="task_home"` (overrides the env's `low_home` default), `camera_kwargs={"camera":"box_detail"}`. (Older simple-pick policies live in `UR3_pick_ppo`; the deployment `download_policy_from_wandb` default points there.)
...
- `robots/UR3e/ur3_realrobot_dependencies.py` — `UR3RealRobotPick`, the UR3 pick analog of `URSim_RTDE_dependencies.py`. Key differences:
  ...
  - `download_policy_from_wandb(run_id, out_dir, entity, project, force=False)` — static helper that fetches the run's `model` artifact (`params.msgpack`) and writes a `load_policy_fn`-ready `metadata.json` alongside it. **Prefers the artifact's `inference_config.json`** for `network_factory` + obs/action sizes + `env_name` (the real trained architecture); falls back to `run.config` `network_factory` + `registry.load(env_name)` dims for older runs that predate self-describing artifacts (warns on obs/act mismatch, and warns when `network_factory` ends up empty — which causes a `params.msgpack` shape error in `load_policy_fn`). No-ops if `out_dir` already has both files unless `force=True`. Default entity/project `weissma6-zhaw-school-of-engineering` / `UR3_pick_ppo` (matches the simple-pick sweep; pick-and-place runs train to `UR3_PicknDrop` — pass `project=` or set `WANDB_PROJECT` for those).
