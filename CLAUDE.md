...
- `manipulation/my_ur3/` — Custom UR3e + Robotiq Hand-E pick environment, mirroring `my_ur10/`:
  ...
  - `ur3_pick.py` — `UR3Pick` env. **Lift-to-an-air-point task** — NOT pick-and-place (that is the sibling `ur3_picknplace.py`; do not conflate them). The box spawns near the keyframe pose with **wide XY jitter** (`±[0.15, 0.10]`, so X∈[0.30,0.60] at `task_home`'s 0.45), an optional **Z-axis yaw** (`box_z_rot_range`, default 0 — this **replaced** the old unphysical ±45° Y-tilt), and optionally rests on a variable-height **lifter plate** (`lifter_height_max`); the policy grasps it and lifts it to a **target point floating ~8–15 cm above the box** (rendered as a red **sphere** marker; raised by the lifter height), holding within **5 mm**. **26D obs** `[q(8 arm+finger), qd(6 arm), (box−tcp)(3), (target_pos−box)(3), box_xmat.ravel()[:6](6)]`, **7D action** (6 arm delta + 1 Hand-E tendon). `ctrl_dt=0.02`, `sim_dt=0.005`, `action_scale=0.04`, `episode_length=150`, `init_keyframe="task_home"` (env default; sweeps may override). Three **curriculum knobs**, all config, driven from the sweep JSONL via `env_overrides`: **`init_start_random`** (`"none"` = literal keyframe start, else `"light"/"mid"/"hard"` = sample one `load_init_poses` library pose per reset), **`lifter_height_max`** (m; per-episode box-riser height, default **0.03**, 0 = box on floor), **`box_z_rot_range`** (rad; box spawn yaw, default 0 = axis-aligned). **Reward = 8 staged terms with sticky latches** (`reached_box`→`grasped`→`lifted`, monotone via `jp.maximum`; restored in `66ad216` after the pipeline split had left 6 ungated terms), scales `{gripper_box:4.0, grasp:3.0, lift:5.0, box_target:8.0, box_orient:2.0, gripper_align:2.0, no_floor_collision:0.25, robot_target_qpos:0.3}`: `grasp`/`lift`/`box_orient` gate on `reached`; `box_target` gates on `lifted` (anti-push — a box slid along the floor earns nothing); **`gripper_align`** (added `e4912b8`) rewards the horizontal jaw axis (vector between the two finger touch sites) lining up with a box face via `cos(4·Δyaw)` — 90°-periodic so all 4 faces of the square cube score max — proximity-weighted by `1−tanh(5·gripper_box_dist)`. `box_orient` still rewards **identity**, so a nonzero `box_z_rot_range` trains the policy to de-rotate the spawn yaw. `box_target` is **two-scale** `0.5·(1−tanh 5d) + 0.5·(1−tanh 30d)` — the fine term supplies a steep near-goal gradient so the box is pulled the last ~cm (plain `tanh 5d` is flat near zero, which had the box plateauing ~13 mm short).
    - **Success** = box-center within `success_tol` (config, default **0.005 m**) of the target for **3 consecutive steps**; episode also ends on `out_of_bounds` (`|tcp.xy| > 0.6` or `tcp.z < 0`) or NaN. There is **no** corner/containment math, `box_inside`, `release`, or `retract` here — those live **only** in `ur3_picknplace.py`; the staged `grasp`/`lift`/`lifted` sticky latches, however, DO exist here (see the reward above). `rot_err` is computed but diagnostic-only (not in success, not in metrics).
    - **Reset** (`reset()`, splits rng into **8**): box XY `init_obj_pos.xy + uniform(±[0.15, 0.10])`; box Z **rests on the lifter** when enabled (`h + plate_half + box_half`, `h ~ uniform(0.003, lifter_height_max)` written to the 2nd mocap body `lifter`) else the keyframe `0.02`; box quat = **Z-yaw** `uniform(±box_z_rot_range)` (identity when 0). Target `init_obj_pos + uniform([+0.01,−0.03,+0.08]‥[0.06,0.03,0.15])` with **Z raised by `h`** so the goal stays above a raised box (X biased forward; envelope kept **reachable** — required for the 5 mm goal; Y pulled in), written to `mocap_pos[mocap_target]` (mocapid **0**) + `info["target_pos"]`. Arm/finger start = a random `load_init_poses(init_start_random, "train")` library pose (or the keyframe when `"none"`), **then** `+ uniform(−v,+v)` from `init_qpos_noise` (a **7-vector**, 6 arm + 1 finger; set to 0 in the curriculum sweeps so the library/keyframe pose is used verbatim).
  ...
  - `xmls/mjx_single_cube_position_ur3.xml` — pick scene: includes the robot + scene, **4×4×4 cm box** freejoint (geom half-extents `0.02`, resting center z=0.02), `mocap_target` body (a **visual sphere** geom, **radius `0.04`** — big enough to contain the 4 cm cube so a lifted box visibly sits inside it; `contype/conaffinity=0`), a **`lifter`** mocap body (thin **20×20 cm, 1 mm** box geom, `contype=2/conaffinity=3` so it holds the box but is a distinct collision category from box/floor — inserted **after** `mocap_target` so the target keeps mocapid **0**, which the deploy render writes by index; positioned per-episode by `reset()`), `ur3_pick_sensor.xml`. Loads to `nu=7, nq=15, nmocap=2, nkey=5, nsensor=8`. Keyframes `task_home`/`low_home`/`tucked` (15 qpos = 6 arm + 2 finger + 7 box; 7 ctrl), identity box orientation; box pose `(0.45, 0, 0.02)` in `task_home` (the sweep keyframe), `(0.3, 0, 0.02)` in `low_home`/`tucked`. (The 5 cm **box** target with corner-containment is `mjx_single_cube_position_ur3_picknplace.xml`, not this file.)
