...
- `robots/URSim/UR_RTDE_Tests.ipynb` — bring-up sanity notebook for the lab **UR3e on PolyScope X 10.12** (`192.168.1.4`), reusing `UR3RealRobotPick` (imported from `../UR3e` via a `sys.path` shim): (1) connect via External Control URCapX (`use_ext_urcap=True`), (2) receive joint/TCP feedback, (3a/3b) Hand-E XML-RPC connect → read native `pos_pct` → open/close to lock the sim↔percent mapping, (4) move arm + hand to the `tucked` keyframe. Run this before a full pick loop.
