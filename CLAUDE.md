...
- `manipulation/my_ur3/` — Custom UR3e + Robotiq Hand-E pick environment, mirroring `my_ur10/`:
  ...
  - `ur3_pick.py` — `UR3Pick` env. **Lift-to-an-air-point task** — NOT pick-and-place (that is the sibling `ur3_picknplace.py`; do not conflate them). The box spawns near the keyframe pose with **wide XY jitter** (`±[0.15, 0.10]`, so X∈[0.25,0.55] at `task_home`'s 0.40 — read the keyframe for the current X, it is actively tuned), an optional **Z-axis yaw** (`box_z_rot_range`, default π/4 — this **replaced** the old unphysical ±45° Y-tilt), and optionally rests on a variable-height **lifter plate** (`lifter_height_max`); the policy grasps it and lifts it to a **target point floating ~12–15 cm above the box** (rendered as a red **sphere** marker; raised by the lifter height), holding within **5 mm**. **26D obs** `[arm_q(6), gripper(1), (box−tcp)(3), (target_pos−box)(3), jaw_proj(3), app_proj(3), last_action(7)]` — 13D base (6 arm joints + combined finger opening + two relative position vectors) + 6D grasp-frame orientation feats (the gripper's jaw axis and approach axis each projected onto the 3 box axes) + the previous raw 7D action (keeps `action_rate` Markov); a **13D** legacy layout (base only) also exists and the deploy obs-builder branches on the policy's `obs_dim`. **7D action** (6 arm delta + 1 Hand-E tendon). `ctrl_dt=0.02`, `sim_dt=0.005`, `action_scale=0.015` (arm) + `gripper_action_scale=0.02`, `episode_length=250`, `init_keyframe="task_home"` (env default; sweeps may override). Three **curriculum knobs**, all config, driven from the sweep JSONL via `env_overrides`: **`init_start_random`** (`"none"` = literal keyframe start, else `"light"/"mid"/"hard"` = sample one `load_init_poses` library pose per reset), **`lifter_height_max`** (m; per-episode box-riser height, default **0.03**, 0 = box on floor), **`box_z_rot_range`** (rad; box spawn yaw, default **π/4≈0.785** — sweeps override, e.g. **2π** for full-rotation coverage). Further config knobs plumbed the same way through `env_overrides`: **`lifter_tilt_max`** (rad; per-episode plate roll/pitch), **`box_y_center_offset`** / **`target_y_center_offset`** (m; lateral shift of the spawn/goal envelope), **`action_scale`**, **`success_tol`**, **`sticky_latches`** (monotone latches vs. live). **Lift-target sampling is selectable via `target_mode`** (static config, branched in `reset()`): **`"box"`** (default, legacy — byte-for-byte unchanged) samples the target with fixed axis-aligned XYZ jitter bands around `self._init_obj_pos` (+ the `*_y_center_offset` transport hack); **`"base_polar"`** instead samples the target XY in an **annulus around the ROBOT BASE** — radius `target_r_min`‥`target_r_max` (default 0.30‥0.48 m, kept ≤ the UR3's ~0.54 m reach), at an azimuth offset `target_azim_min`‥`target_azim_max` (default π/2‥π) to a random side of the box's bearing from the base — so the goal tracks the jittered box instead of a fixed anchor (this **supersedes** `box_y_center_offset`/`target_y_center_offset`). The **base XY is sourced from the model** (`self._robot_base_xy = mj_model.body("base").pos[:2]`), not assumed to be the world origin. Z is unchanged from `"box"` (the fixed 0.18‥0.21 band + `lifter_h`); a `HEIGHT NOTE` comment marks where to swap in an elevation angle for a true reach sphere. The target stays a **single 3D point**, so the reward/obs/deploy path is untouched. **Reward = 10 staged terms with sticky latches** (`reached_box`→`grasped`→`lifted`, monotone via `jp.maximum` when `sticky_latches`), scales `{gripper_box:4.0, approach_open:1.0, grasp:3.0, lift:5.0, box_target:20.0, hold_target:6.0, gripper_align:5.0, no_floor_collision:0.25, robot_target_qpos:0.3, action_rate:-0.10}` (box_target 8→20 + hold_target 3→6 rebudget `8bb664a`; action_rate -0.01→-0.05→-0.10 smoothness). There is **no `box_orient`** term (the old de-rotate-to-identity reward was removed). **`gripper_align`** is the grasp-frame alignment: a **jaw axis** (between the two finger touch sites) and an **approach axis** (palm→fingertips) are each projected onto the 3 box axes and scored `max(|proj|)` through a 60° soft cone; `alignment = jaw_score·app_score` — both must line up, so "2 of 3 gripper axes aligned ⇒ the 3rd is forced by orthonormality". The `max`+`abs` makes it **invariant to the cube's 24-fold octahedral symmetry** (every equivalent grasp — any 90°/180° yaw, any face — scores the same, in full 3-D, not just yaw), proximity-gated by `1−tanh(3·gripper_box_dist)`; the same 6 projection numbers are the obs orientation feats. The `grasped` latch is **gated on `alignment > grasp_align_thresh`** so a misaligned grab can't unlock the lift/transport chain. `approach_open` keeps the hand open until reached; `hold_target` is a dwell-time bonus for keeping a lifted box in the goal sphere (doubles as a drop penalty); `action_rate` is a squared-Δaction smoothness penalty (needs `last_action` in the obs). `box_target` is **two-scale** `0.5·(1−tanh 5d) + 0.5·(1−tanh 30d)` — the fine term supplies a steep near-goal gradient so the box is pulled the last ~cm (plain `tanh 5d` is flat near zero, which had the box plateauing ~13 mm short).
    - **Physics domain randomization** (config `domain_rand`, **per-EPISODE, in-env**, every axis default **OFF → byte-for-byte unchanged when disabled** via a static master gate): sampled fresh in `reset()` (`_sample_physics_dr`, stashed in `info`) and applied to a per-episode model via `self._mjx_model.tree_replace(...)` at the top of `step()` (`_randomize_physics`) — deliberately **NOT** the brax per-env `randomization_fn` wrapper (`make_domain_randomize_fn` in `run_experiment.py` is superseded for UR3; do not enable both). Independently-gated axes for one-at-a-time ablation: `cube_mass`/`cube_friction`/`cube_size` (box `body_mass`+`body_inertia` / `geom_friction[0]` / `geom_size` with the width half-extent hard-clamped ≤0.018 so it stays graspable), `gravity` (`opt.gravity` magnitude ± directional tilt), `arm_stiffness`/`arm_damping` (× the 6 arm position-actuator kp `actuator_gainprm[:6,0]` / kv `actuator_biasprm[:6,2]` — raw arm `jnt_stiffness`/`dof_damping` are **0** in the model, so DR scales the servo gains, not passive joint fields). Realized factors are logged **terminal-gated** as `dr/*` (so brax's EvalWrapper episode-sum = the single value). **No obs change** (robustness DR, not privileged-info adaptation). Commented stubs (off) for restitution/joint_limits/per-step cube force/action delay. Sweep-overridable via **dotted keys** (`domain_rand.enable`, `domain_rand.cube_mass.enable`/`.min`/`.max`, …).
...
- `batch_runs/sweeps/UR3Pick_sweep.jsonl` — **simple pick** sweep (`env_name="UR3Pick"`, `wandb_project="UR3_pick_ppo"`). Contents are **volatile** (user-edited per experiment) — read the file for the current configs, don't trust a line count here. **Currently a 1-line run** (`Spheretarget_mid_30M`, `--array=1-1%1`): the new **`target_mode="base_polar"`** sphere-target scheme (annulus around the base) at `init_start_random="mid"` randomness (lifter≤2 cm, `box_z_rot_range=2π`), 30M-step target (`num_minibatches=32` lands TOTAL≈31.9M), `lr 6e-4`, `action_scale=0.025`.
...
- **UR3 grasp-hold contact tuning** (commit `a6cd065`, **REVERTED by `ea1ffd2`**): a6cd065 stiffened the grasp contacts to stop the lifted box dropping — scene `<option>` `impratio="10"` and box + finger `solref="0.005 1"` — but it **jammed the gripper (fingers never opened)** and was reverted. **Current / working values:** `impratio` is **unset → MuJoCo default 1** (pyramidal cone), box (`mjx_single_cube_position_ur3.xml`) and both finger geoms (`ur3e_position.xml`) use **`solref="0.01 1"`**, floor stays stiffer at `0.001 1`. Lesson from the revert: keep `impratio` moderate and do NOT push `solref` below ~2× the `0.005 s` timestep (0.005 was 1× → too stiff). Holding force is NOT the bottleneck (≈64 g box vs. `kp=400`/`forcerange=-130 130`). These `<option>`/geom XMLs are **shared with `UR3PicknPlace`**, so any contact-physics change affects both envs and **requires SLURM retraining** since the policy is trained on the sim's physics.
