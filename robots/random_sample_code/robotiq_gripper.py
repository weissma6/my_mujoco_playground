"""Socket client for a Robotiq gripper (e.g. Hand-E) on a UR controller.

Vendored from the canonical SDU Robotics / ur_rtde example
(`robotiq_gripper.py`). Talks the Robotiq URCap socket protocol on the UR
controller (default port 63352) using only the Python stdlib `socket`.

Position convention (Robotiq native): 0 = fully OPEN, 255 = fully CLOSED.
Requires the Robotiq URCap to be installed and running on the teach pendant.
"""
...
class RobotiqGripper:
    ...
    ENCODING = "UTF-8"  # ASCII and UTF-8 both work here
    ...
    def move_and_wait_for_pos(
        ...
        that the move had completed, a status indicating how the move ended (see ObjectStatus enum for details).
        """
